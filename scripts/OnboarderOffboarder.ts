...
const LENDING_TERM = '0x7F021599B52C206D736735894f5E726e8AfBA394'; // this is the lending term that will be onboarded/offboarded
...
async function Onboard() {
  const onboardingFactory = LendingTermOnboarding__factory.connect(ON_BOARDING_CONTRACT, web3Provider);

  (await onboardingFactory.proposeOnboard(LENDING_TERM)).wait();

  // uint256 proposalId = hashProposal(targets, values, calldatas, keccak256(bytes(description)));
  const proposalParams = await onboardingFactory.getOnboardProposeArgs(LENDING_TERM);
  const proposalId = await onboardingFactory.hashProposal(
    proposalParams.targets,
    proposalParams.values,
    proposalParams.calldatas,
    ethers.keccak256(proposalParams.description)
  );

  await onboardingFactory.castVote(proposalId, 10_000_000n * 10n ** 18n);
}
