...
export class LinkWalletCommand {
  static cmd: ApplicationCommandData = {
    name: 'link-wallet',
    description: 'Link a public address to your discord username. allowing other calls after',
    options: [
      {
        name: 'address',
        description: 'Address to link',
        required: true,
  ...
  static async execute(interaction: CommandInteraction) {
    ...
    const msg = this.linkWallet(interaction.user.id, interaction);
